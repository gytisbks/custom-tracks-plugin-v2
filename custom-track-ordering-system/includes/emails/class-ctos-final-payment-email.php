<?php
/**
 * Class for the final payment email.
 */
if (!defined('ABSPATH')) {
    exit; // Exit if accessed directly
}

if (!class_exists('CTOS_Final_Payment_Email')) :

/**
 * Final Payment Email
 *
 * Email sent to producers when a customer makes the final payment.
 */
class CTOS_Final_Payment_Email extends WC_Email {
    /**
     * Constructor
     */
    public function __construct() {
        $this->id = 'ctos_final_payment';
        $this->title = __('Custom Track Final Payment', 'custom-track-ordering-system');
        $this->description = __('This email is sent to producers when a customer makes the final payment for a custom track.', 'custom-track-ordering-system');
        
        $this->template_html = 'emails/final-payment.php';
        $this->template_plain = 'emails/plain/final-payment.php';
        $this->template_base = CTOS_PLUGIN_DIR . 'templates/';
        
        $this->subject = __('Final payment received for order {order_number}', 'custom-track-ordering-system');
        $this->heading = __('Final payment received', 'custom-track-ordering-system');
        
        // Call parent constructor
        parent::__construct();
        
        // Triggers for this email
        add_action('ctos_order_status_awaiting_final_delivery', array($this, 'trigger'), 10, 1);
    }
