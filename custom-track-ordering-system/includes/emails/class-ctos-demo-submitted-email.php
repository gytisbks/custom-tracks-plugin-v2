<?php
/**
 * Class for the demo submitted email.
 */
if (!defined('ABSPATH')) {
    exit; // Exit if accessed directly
}

if (!class_exists('CTOS_Demo_Submitted_Email')) :

/**
 * Demo Submitted Email
 *
 * Email sent to customers when a producer uploads a demo track.
 */
class CTOS_Demo_Submitted_Email extends WC_Email {
    /**
     * Constructor
     */
    public function __construct() {
        $this->id = 'ctos_demo_submitted';
        $this->title = __('Custom Track Demo Submitted', 'custom-track-ordering-system');
        $this->description = __('This email is sent to customers when a producer uploads a demo track.', 'custom-track-ordering-system');
        
        $this->template_html = 'emails/demo-submitted.php';
        $this->template_plain = 'emails/plain/demo-submitted.php';
        $this->template_base = CTOS_PLUGIN_DIR . 'templates/';
        
        $this->subject = __('Your custom track demo is ready for review - Order {order_number}', 'custom-track-ordering-system');
        $this->heading = __('Your custom track demo is ready!', 'custom-track-ordering-system');
        
        // Call parent constructor
        parent::__construct();
        
        // Triggers for this email
        add_action('ctos_order_status_awaiting_customer_approval', array($this, 'trigger'), 10, 1);
    }
